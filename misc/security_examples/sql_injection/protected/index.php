...
<html>
  ...
  <body>
    <h3>Search Transactions (Protected Version)</h3>
    <a href="../../">Security Example Index</a> | <a href="../">Go to Insecure Version</a> | <a href="reset.php">Reset Data</a>
    <?php
      ...
      if (isset($_GET['search_term']))
      {
        ...
        // Prepare and execute a statement, using placeholders for values
        $query_string = "SELECT * FROM sqli_transaction 
                         WHERE (sent_by = ? OR sent_to = ?) AND description LIKE ?
                         ORDER BY pay_date";
        $stmt = $db->prepare($query_string);
        $stmt->execute( [$_SESSION['uname'], $_SESSION['uname'], $search_term] );
